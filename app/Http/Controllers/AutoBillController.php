				...
				if($client->package_change==1){
					...
					if($package_change_date == $this_year."-".$this_month){
						...
						if($change_package_data){
							...
							$new_package_data   = Packages::find($change_package_data->new_package_id);
							$old_package_data   = Packages::find($change_package_data->old_package_id);
							
							if($start_of_month==$client->package_change_date){
								$client->package_price = $change_package_data->package_price;
							}
							else{
							...
							Clients::where("auth_id",$client->auth_id)->update(["package_change"=>0]);
						}
					}
				}
