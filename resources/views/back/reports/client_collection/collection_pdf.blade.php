               ...
               {{--</table>--}}
                ...
                <table class="table">
                    ...
                    @foreach($all_collection["collection"] as $key=>$collection)
                        ...
                        $discount = $collection->discount_amount + $collection->permanent_discount;
                        $total_discount+=$discount;
                        @endphp
                        <tr>
                            <td class="text-center">{{ $key+1 }}</td>
                            <td class="text-center">{{ date("d/m/Y", strtotime($collection->receive_date)) }}</td>
                            <td class="text-left">{{ $collection->client->zone->zone_name_en }}</td>
                            <td>{{ $collection->client->client_id}}-{{ $collection->client->client_name }}</td>
                            <td class="text-right">{{ number_format($discount,2) }}</td>
                            <td class="text-right">{{ number_format($collection->receive_amount,2) }}</td>
                            <td>{{ ($collection->receiver_by!="" ? $collection->employee->emp_id .'-'. $collection->employee->emp_name:"")  }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
